...
static int sysv_mknod(struct inode * dir, struct dentry * dentry, int mode, dev_t rdev, void *label)
{
	struct inode * inode;
	int err;
...
{
  return sysv_mknod(dir, dentry, mode, 0, NULL);
}
...
static int sysv_mkdir(struct inode * dir, struct dentry *dentry, int mode, void *label)
{
	struct inode * inode;
	int err = -EMLINK;
